...
const senatorImages = {
  ...
  'Bob Casey Jr.': 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/Bob_Casey_Jr._official_photo_%28cropped%29.jpg/190px-Bob_Casey_Jr._official_photo_%28cropped%29.jpg',
  ...
  'Jeanne Shaeen': 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/68/Shaheen_Senate_Portrait_%28cropped%29_%28cropped%29.jpg/190px-Shaheen_Senate_Portrait_%28cropped%29_%28cropped%29.jpg',
  ...
  'John Barasso': 'https://upload.wikimedia.org/wikipedia/commons/thumb/4/42/John_Barrasso_official_portrait_112th_Congress_%28cropped%29.jpg/190px-John_Barrasso_official_portrait_112th_Congress_%28cropped%29.jpg',
  ...
  'Markwayne Mullin' : 'https://upload.wikimedia.org/wikipedia/commons/thumb/b/b0/Markwayne_Mullin_official_Senate_photo_%28cropped%29.jpg/190px-Markwayne_Mullin_official_Senate_photo_%28cropped%29.jpg',
...
function App() {
    ...
    const handleSubmit = () => {
      const maxDistance = Math.sqrt(5); // Maximum possible distance in a 5D space
    
      // Check if all user inputs are 0
      const allZeros = Object.values(sliderValues).every((value) => value === 0);
    
      if (allZeros) {
        const updatedSenators = senators.map((senator) => ({
          ...senator,
          score: 0, // Set all scores to 0
        }));
      ...
      const updatedSenators = senators.map((senator) => {
        ...
        // Filter out dimensions where the user input is 0
        const filteredDimensions = userVector.reduce((acc, userValue, index) => {
          if (userValue !== 0) { // Include this dimension only if the user input is non-zero
            acc.user.push(userValue);
            acc.senator.push(senatorVector[index]);
          }
          return acc;
        }, { user: [], senator: [] });
    
        // Calculate the Euclidean distance for filtered dimensions
        const distance = Math.sqrt(
          filteredDimensions.user.reduce((sum, userValue, index) => {
            const diff = userValue - filteredDimensions.senator[index];
            return sum + diff * diff;
          }, 0)
        );
    
        // Compute alignment score (normalized to percentage)
      ...
      <div className="subtitle-container">
        ...
        <div className="sliders-container">
          {[
            ...
            {
              id: 'q3',
              left: 'The United states should immediately issue a resolution stating that the war between Israel and Hamas was the product of Israeli occupation, that Israel bears sole responsibility for all violence which has taken place between each side, and that Palestine has a legitimate land claim to all territory currently occupied by Palestinians or Israelis.',
              right: "The United States should immediately issue a resolution stating that the war between Israel and Hamas was exclusively the product of Hamas's aggression, that Hamas and Hezbollah bear sole responsibility for all violence which has taken place, and that Israel has a permanent territorial claim to Gaza and the West Bank.",
            },
            {
              id: 'q4',
